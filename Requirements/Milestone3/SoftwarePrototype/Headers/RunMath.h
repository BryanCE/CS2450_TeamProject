...
// Then takes that value and the value in the register and adds them. This new value OVERWRITES the register variable.
	int Add(int addr, int regVal, int &regOverflow);
	...
	int Subtract(int addr, int regVal, int &regOverflow);
	...
	int Divide(int addr, int regVal, int &regOverflow);
	...
	int Multiply(int addr, int regVal, int &regOverflow);
};
